"""Build idempotency tags for approved orders.

Each approved order carries a `tag` derived from the proposal's
strategy_version_id, market_state_hash, side, and bar_ts. Alpaca's
`client_order_id` rejects re-submission of the same tag, which is our
...
def make_tag(
    *,
    ...
    """Return the idempotency `client_tag` for a proposal.
